...
import { Heading,Image, Button, Stack, Container, Box, Center } from '@chakra-ui/react'
...
import { URL_CART,URL_ONME,BASE_URL, URL_DVTO } from '../api/urls'
...
export default function QR() {
  ...
  const [categoryDefault, setcategoryDefault] = useState(JSON.parse(params))
  ...
  const id_costomer = (param.get('customer_id') === 'null' || param.get('customer_id') === null) ? Math.round(min + Math.random() * (max - min)) : param.get('customer_id')
  ...
  useEffect(() => {
    intervalOnme = setInterval(async () => {
          //cek poto exist
        // const requestOptions = {
        //   method: 'GET',
        // }
        var image_data_url = "";
        // fetch(URL_ONME_POTO+id_costomer, requestOptions)
        // .then(response => response.json())
        // .then(async response => {
            const r = await fetch(`/api/img_to_base64?src=${URL_ONME}/filemvto/${id_costomer}.png`)

            image_data_url = await r.text()
            if(image_data_url.includes('data:image/png;base64')){
              // image_data_url = response.data_gambar
    ...
    <>
        ...
        <>
          <Box
            ...
            style={{
              ...
              transformOrigin: '105px 229px 0px',
            }}
          >
            <Container centerContent maxW="xl" height="80vh" pt="0" pb="0" bg="#e3e3e3">
              <div style={{ margin: '0 auto', maxWidth: 154, width: '100%', marginTop: '80px' }}>
                <QRCode
                  ...
                  value={`${URL_DVTO}/?cat=${cat}&login=${!categoryDefault.login ? 'false' : categoryDefault.login}&customer_id=${id_costomer}&id_cat=${id_cat}&id_prod=${id_prod}&prod_name=${prod_name}&onme=on`}
                />
              </div>
              <Center>
                <Box height={105}>
              ...
              <Box style={{ backgroundColor: '#FFFFF', position: 'absolute', bottom: '70px', left: '10px' }} display={'block'}>
                <Link
                  href={{
                    pathname: '/drag-area',
                ...
                >
                  <Image alt="" h="50px" objectFit="cover" id="emojimage" display={'block'} />
                </Link>
              </Box>
              <Center>
                <Box width={305}>Scan the QR code to Try On using your phone camera. Refresh and try again if you face any issues.</Box>
              </Center>
            </Container>
          </Box>
        </>
      ...
      <Box w={'100%'} maxH={350} h={210} bg="whiteAlpha" bottom={0} position="fixed" id="box-footer" zIndex={1900}>
        ...
        {/* <Box pb={0} pt={0} w={"100%"} h={180} bg='#FFFFFF' bottom={0} position="relative">
              <Box h="20px" id="box-title-product"
                  position={"absolute"}
                  left={"20px"}
                  top={"70px"}
                  >
              </Box>
              <Heading id="box-price-product"
                  as="h5"
                  size="smll"
