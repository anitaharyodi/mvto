  ...
  const dimensiProduct = React.useMemo(() => {
    ...
    if (key_model !== '') {
      if (on_me === "on") {
        ...
        pw = KoordinatsModels[key_model].model.marginMobile.dimension[index_selected_model] + '%'
      } else {
        pw = KoordinatsModels[key_model].model.marginDesktop.dimension[index_selected_model] + '%'
      }
    }
    ...
    <>
      ...
      <Moveable
        ...
        onRender={e => {
          ...
          let rotates=0
          if (current_cat === CAT_BRACELETS && on_me == 'on') {
            rotates = -110
          }else{
            rotates = rotate
          }
          e.target.style.transformOrigin = transformOrigin
          e.target.style.transform = `translate(${translate[0]}px, ${translate[1]}px)` + ` rotate(${rotates}deg)`
