...
import { RiAlignJustify, RiCameraFill, RiCameraOffFill, RiStackFill, RiCoinFill, RiArrowLeftSLine, RiSunFill, RiSunLine, RiAddCircleFill, RiIndeterminateCircleFill, RiCloseCircleFill, RiRotateLockLine, RiRotateLockFill } from "react-icons/ri"
...
export default function Rings(params) {
      ...
      if (faces.length > 0) {
        ...
        if (scaleWidth > 412 || scaleWidth > 450) {
          ...
          if ((FaceBottomRight[0] >= 370 && FaceBottomRight[0] < 430) && (FaceTopLeft[0] >= 380 && FaceTopLeft[0] < 400)){
            ...
            const IndexParamsLS=localStorage.setItem('IndexParams',0)
            const newParams={
  ...
  function turnOffCamera() {
    ...
    const paramLS=localStorage.setItem('params',JSON.stringify(newParams))
    !imageCapture.img ? (setImageCapture({ image:KoordinatsModelsEarings.KoordinatEarings.model.image })) : (imageCapture)
  ...
  return (<>
    {camState === "on" ? (<>
      <Box w={"100%"} bg='#0e1c28f0' id="box-onmodel"
        style={{
      ...
      >
        ...
        >
          {<svg
            width="100%" height="100%" viewBox="0 0 372 432" fill="none" xmlns="http://www.w3.org/2000/svg"
            style={{
              top: "-50px",
              left: "40px",
              position: "fixed",
          ...
          >
            <path
              className={actionOncahnge === "Manual" ? (
                animasiClassOf
              ) : (
                animasiClassOf
              )
        ...
        <Box style={{ backgroundColor: "#FFFFF", position: "absolute", bottom: "70px", left: "10px" }}
          display={
            camState === "off" ? (
              "none"
            ) : (
        ...
        >
          <Link href={{
            ...
            <Image h="50px" objectFit="cover" id="emojimage"
              display={
                camState === "off" ? (
                  "none"
                ) : (
          ...
          <Box margin={2} padding='4' maxW='md'>
            ...
            <Stack spacing={0} direction="row" align="center" marginBottom={6} marginLeft={6}>
              <Button
                ...
                borderRadius={"50%"}
                leftIcon={
                  actionOncahnge === "Manual" ? (
                    <RiCameraFill size={25} style={{ marginLeft: '6px' }} />
                  ) : (
                    <RiCameraOffFill size={25} style={{ marginLeft: '6px' }} />
                  )
                }
                disabled={
                  actionOncahnge === "Manual" ? (disabledbottonCamera?true:false) : (true)
                }
                onClick={
                  actionOncahnge === "Manual" ? (takeCap) : (turnOffCamera)
                }
                colorScheme={
                  actionOncahnge === "Manual" ?
                    ("gray") : ("blackAlpha")
                }
                bg={actionOncahnge === "Manual" ?
                  ("#FFFFFF") : ("rgba(0, 0, 0, 0.92)")
                }
              >
              </Button>
            </Stack>
    ...
    {camState === "off" ? (<><Footer params={{ OnMe: RingOnMe, img: imageCapture,ProductImageThumb:imageStackLS.ProductImageThumb.Earings,ProductImage:imageStackLS.ProductImage.Earings,productName:imageStackLS.productName.Earings,productPrice:imageStackLS.productPrice.Earings,productID:imageStackLS.productID.Earings }} /></>) : (<></>)}
  </>
  )
}